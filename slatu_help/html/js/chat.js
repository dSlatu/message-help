window.APP = {
	...
	mounted() {
		post('https://kosmos_interactloaded', JSON.stringify({}));
	...
	methods: {
		...
		send(e) {
			if(this.message !== '') {
				post('https://kosmos_interactchatResult', JSON.stringify({
		...
		hideInput(canceled = false) {
			if (canceled) {
				post('https://kosmos_interactchatResult', JSON.stringify({ canceled }));
			}
			this.message = '';
